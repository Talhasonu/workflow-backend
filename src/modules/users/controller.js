...
const { workspaceService, authService, emailService } = require('../../services');
...
const forgotPassword = catchAsync(async (req, res) => {
    const validationResult = userValidation.validateForgotPasswordInputs(req.body);
    if (validationResult?.error) {
        return res.status(httpStatus.BAD_REQUEST).send({
            isSuccess: false,
            message: validationResult.msg,
    ...
    const { user, resetToken } = await userService.createForgotPasswordToken(validationResult.value.email);
...
const verifyEmail = catchAsync(async (req, res) => {
    ...
    const verificationData = await userService.verifyEmailToken(validationResult.value.token);
    res.send({
        success: true,
...
const getWorkspaceUsers = catchAsync(async (req, res) => {
    ...
    const validationResult = userValidation.validateGetWorkspaceUsersQuery(req.query);
    if (validationResult?.error) {
        return res.status(httpStatus.BAD_REQUEST).send({
            isSuccess: false,
            message: validationResult.msg,
...
const changePassword = catchAsync(async (req, res) => {
    const validationResult = userValidation.validateChangePasswordInputs(req.body);
    if (validationResult?.error) {
        return res.status(httpStatus.BAD_REQUEST).send({
            isSuccess: false,
            message: validationResult.msg,
...
const updateUser = catchAsync(async (req, res) => {
    ...
    const updatedUser = await userService.updateUserById(req.user._id, validationResult.value);
...
const inviteTeamMember = catchAsync(async (req, res) => {
    const validationResult = userValidation.validateInviteTeamMemberInputs(req.body);
    if (validationResult?.error) {
        return res.status(httpStatus.BAD_REQUEST).send({
            isSuccess: false,
            message: validationResult.msg,
...
const acceptInvitation = catchAsync(async (req, res) => {
    const validationResult = userValidation.validateAcceptInvitationInputs(req.body);
    if (validationResult?.error) {
        return res.status(httpStatus.BAD_REQUEST).send({
            isSuccess: false,
            message: validationResult.msg,
    ...
    const member = await userService.acceptInvitationByToken(validationResult.value);
